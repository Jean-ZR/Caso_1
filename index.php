...
// Función para generar un nombre de carpeta aleatorio
function generarNombreCarpeta() {
    return bin2hex(random_bytes(3)); // Genera una cadena aleatoria
}

// Verificar si ya se ha creado un nombre de carpeta en la sesión
if (!isset($_SESSION['carpetaNombre'])) {
    $carpetaNombre = generarNombreCarpeta();
    $_SESSION['carpetaNombre'] = $carpetaNombre;
} else {
    $carpetaNombre = $_SESSION['carpetaNombre'];
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_FILES['archivo'])) {
            foreach ($_FILES['archivo']['name'] as $key => $name) {
                ...
                $archivoRuta = $carpetaRuta . '/' . basename($name);

                if (move_uploaded_file($tmp_name, $archivoRuta)) {
                    $subido = true;
    ...
    if (isset($_POST['eliminarArchivo'])) {
        ...
        if (file_exists($archivoRutaAEliminar)) {
            if (is_file($archivoRutaAEliminar)) {
                ...
                throw new Exception("'$archivoAEliminar' no es un archivo válido para eliminar.");
            }
        } else {
